...
public class FlowLayout extends ViewGroup {
    ...
    public FlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        tagMarkViewId = ta.getResourceId(R.styleable.FlowLayout_tagMarkViewId, View.NO_ID);
    ...
    public void setTags(List tags, List selectedList) {
        ...
        if (tags != null && tags.size() > 0) {
            ...
            for (Object tag : tags) {
                ...
                vMark = tagMarkViewId == NO_ID ? null : rootView.findViewById(tagMarkViewId);
                if (vMark != null) {
                    boolean showTag = false;
                    if (tag instanceof TagModel) {
                        showTag = ((TagModel) tag).getShowMarker();
                    } else {
                        try {
                            Method method = tag.getClass().getDeclaredMethod("getShowMarker");
                            method.setAccessible(true);
                            showTag = (boolean) method.invoke(tag);
                        } catch (Exception ignore) {
                        }
                    }
                    vMark.setVisibility(showTag ? View.VISIBLE : View.GONE);
                }
                Class<?> tagModelClass;
                try {
                ...
                if (tagModelClass != null && tagModelClass.isAssignableFrom(tag.getClass())) {
                    rootView.setSelected(((TagModel) tag).isSelected());
                }
     ...
     * @param listener        点击监听。
     */
    public void setOnTagClickListener(boolean clickRepeatable, OnTagClickListener listener) {
    ...
    public interface OnTagClickListener {
        void onTagClick(FlowLayout parent, View tagView, Object tag, int index);
    }
}
