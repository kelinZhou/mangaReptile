...
public class SwipeBackLayout extends FrameLayout {
    ...
    private void init() {
        ...
        setShadow(R.drawable.im_shadow_left, EDGE_LEFT);
        setEdgeOrientation(EDGE_LEFT);
    }

    /**
     * Set scroll threshold, we will close the context, when scrollPercent over
     * this value
     *
     * @param threshold
     */
    ...
    public void setEdgeOrientation(int orientation) {
        ...
        if (orientation == EDGE_RIGHT || orientation == EDGE_ALL) {
            setShadow(R.drawable.im_shadow_right, EDGE_RIGHT);
        }
    }
    ...
    class ViewDragCallback extends ViewDragHelper.Callback {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            ...
            if (dragEnable) {
                ...
                if (mPreFragment == null) {
                    if (mFragment != null) {
                        @SuppressLint("RestrictedApi") List<Fragment> fragmentList = mFragment.getFragmentManager().getFragments();
        ...
        public void onViewPositionChanged(View changedView, int left, int top, int dx, int dy) {
            ...
            if (mListeners != null && !mListeners.isEmpty() && mHelper.getViewDragState() == STATE_DRAGGING && mScrollPercent <= 1 && mScrollPercent > 0) {
                for (OnSwipeListener listener : mListeners) {
        ...
        public int getViewHorizontalDragRange(View child) {
            if (mFragment != null) {
                return 1;
            } else if (mActivity != null && mActivity instanceof SwipeBackFragmentActivity && ((SwipeBackFragmentActivity) mActivity).swipeBackPriority()) {
                return 1;
            } else if (mActivity != null && mActivity instanceof SwipeBackActivity && ((SwipeBackActivity) mActivity).swipeBackPriority()) {
                return 1;
            }
