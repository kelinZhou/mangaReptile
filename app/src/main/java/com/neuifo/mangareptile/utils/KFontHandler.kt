...
class KFontHandler private constructor(private var onTextClickListener: ((flag: String) -> Unit)? = null) :
    ...
    companion object {
        ...
        fun loadResource(
            ...
            return htmlFrom(
                getStringFromStream(context.resources.openRawResource(resId), split, partIndex),
                textClickListener
            )
        }

        private fun htmlFrom(
            source: String,
            textClickListener: ((flag: String) -> Unit)? = null
        ): CharSequence {
        ...
        private fun getStringFromStream(
            ...
            return when {
                split.isEmpty() -> sb.toString()
                partIndex == 0 -> sb.split(split)[0]
                else -> {
                    sb.split(split).subList(partIndex, sb.split(split).size).joinToString(split)
                }
            }
        }
    }
