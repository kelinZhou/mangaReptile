...
abstract class SuperRecyclerViewAdapter<T, VH>(
     ...
     */
    private var onLoadMoreListener: ((isReload: Boolean) -> Unit)? = null
    ...
    open fun addItem(position: Int, itemData: T) {
        dataList.add(position, itemData)
        notifyItemInserted(position)
    }

    open fun addItem(position: Int, itemData: List<T>) {
        dataList.addAll(position, itemData)
        notifyItemRangeInserted(position, itemData.size)
    }

    fun addItem(itemData: T) {
        addItem(itemCount, itemData)
    }

    fun notifyItem(itemData: T) {
        val itemPosi = getItemPosi(itemData)
        dataList[itemPosi] = itemData
        notifyItemChanged(itemPosi)
    }
     ...
     */
    fun setLoadMoreView(@LayoutRes loadMoreLayoutId: Int, @LayoutRes retryLayoutId: Int, listener: ((isReload: Boolean) -> Unit)) {
     ...
     */
    fun setLoadMoreView(
        @LayoutRes loadMoreLayoutId: Int, @LayoutRes retryLayoutId: Int, @LayoutRes noMoreDataLayoutId: Int, @Size(
            min = 1,
            max = 10
