...
abstract class BaseFragmentPresenter<V : BaseViewDelegate<VC>, VC : BaseViewDelegate.BaseViewDelegateCallback> : SwipeBackFragment(),
    ViewPresenter<VC> {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        if (!this.hasViewDelegate) {
            ...
            try {
                ...
                vd.onCreate(this)
                val dv = vd.onCreateView(inflater, container, this.getSavedInstanceState(savedInstanceState))
                vd.bindView(this)
                this.rootView = if (canSwipe) {
                    attachToSwipeBack(dv)
                } else {
                    dv
                }
