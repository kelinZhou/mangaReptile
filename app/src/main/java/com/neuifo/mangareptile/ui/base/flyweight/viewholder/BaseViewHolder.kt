...
abstract class BaseViewHolder protected constructor(view:View, initEnable: Boolean) : RecyclerView.ViewHolder(view), LayoutContainer {
    ...
    protected fun initViewHolder() {
        if (haveItemClickBg) {
            val itemClickView = if (itemClickableViewId != 0 && itemView.findViewById<View>(itemClickableViewId) != null) itemView.findViewById(itemClickableViewId) else itemView
