package com.neuifo.mangareptile.ui.base.flyweight.viewholder
        ...
        if (itemClickable) {
            ...
            clickableView.setOnClickListener {
                if (adapterPosition >= 0) {
                    ...
                    if (it === itemView || it.id == itemClickableViewId) {
                        itemEventListener?.onItemClick(adapterPosition, data)
                    } else {
                        itemEventListener?.onItemChildClick(it, adapterPosition, data)
                    }
                }
            }
            if (itemLongClickable) {
                clickableView.setOnLongClickListener {
                    val data = itemView.getTag(KEY_ITEM_MODEL) as T
                    itemEventListener?.onItemLongClick(adapterPosition, data)
                    true
                }
            }
        }
