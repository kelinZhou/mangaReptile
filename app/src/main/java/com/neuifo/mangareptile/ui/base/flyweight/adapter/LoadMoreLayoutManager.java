...
final class LoadMoreLayoutManager {
     ...
     * 设置状态为加载更多。
     */
    void setLoadState(boolean force) {
    ...
    View getLayoutView(@NonNull ViewGroup parent, View.OnClickListener retryClickListener) {
        ...
        mLoadMoreLayout = addView(mLoadMoreLayoutId, mRootView, inflater, true);
        mRetryLayout = addView(mRetryLayoutId, mRootView, inflater, false);
        if (mRetryLayout != null) {
            mRetryLayout.setOnClickListener(retryClickListener);
        }
        mNoMoreDataLayout = addView(mNoMoreDataLayoutId, mRootView, inflater, false);
        switch (mCurState) {
            case STATE_FAILED:
                setVisible(mRetryLayout, mLoadMoreLayout, mNoMoreDataLayout);
                break;
            case STATE_NO_MORE:
                setVisible(mNoMoreDataLayout, mRetryLayout, mLoadMoreLayout);
                break;
            case STATE_LOAD:
                setVisible(mLoadMoreLayout, mNoMoreDataLayout, mRetryLayout);
                break;
        }
        return mRootView;
    }
     ...
     */
    private View addView(int layoutId, ViewGroup parent, LayoutInflater inflater, boolean resetParentSize) {
        if (layoutId == 0) {
            TextView emptyChild = new TextView(parent.getContext());
            parent.addView(emptyChild, new ViewGroup.LayoutParams(0, 0));
            return emptyChild;
        } else {
