 ...
 * **版本:** v 1.0.0
 */
class SamSungSettingsIntentGenerator(permission: Permission?) : SettingIntentGenerator(permission) {
    override fun onGeneratorDangerousIntent(context: Context): Intent {
        return if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            Intent().apply {
                data = Uri.parse("package:" + context.packageName)
                component =
                    ComponentName("com.android.settings", "com.android.settings.Settings\$AppOpsDetailsActivity")
            }
        } else {
